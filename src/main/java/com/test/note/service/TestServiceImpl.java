/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package com.test.note.service;

import com.test.note.modele.Test;
import com.test.note.repository.TestRepository;
import java.util.List;
import lombok.AllArgsConstructor;
import org.springframework.stereotype.Service;

/**
 *
 * @author FOFANA BASSIROU
 */
@Service
@AllArgsConstructor
public class TestServiceImpl implements TestService{

    private final TestRepository testRepository;


    @Override
    public Test creer(Test test) {
        return testRepository.save(test);
    }

    @Override
    public List<Test> lire() {
         return testRepository.findAll();
    }
    
    @Override
    public Test lireId(Long id) {
         return testRepository.findById(id).orElse(null);
    }

    @Override
    public Test modifier(Long id, Test test) {
        return testRepository.findById(id).map(
        (t) -> {
                t.setNom(test.getNom());
                t.setPrenom(test.getPrenom());
                t.setProfession(test.getProfession());
                return testRepository.save(t);
            }).orElseThrow(()-> new RuntimeException("Test non trouvé"));
    }

    @Override
    public String supprimer(Long id) {
        testRepository.deleteById(id);
        return "Test Supprimé!";
    }
    
}
